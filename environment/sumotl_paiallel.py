...
class SUMOTrafficLights(Environment):
    ...
    * the transitions between states are deterministic
    ...
    def save_vehicle_data(self, produced_data, departed_data):
        ...
        with FileLock("data/produced_vehicles.xlsx.lock"):
            produced_df = pd.DataFrame(produced_data, columns=['Time', 'Produced Vehicles'])
            if os.path.exists('data/produced_vehicles.xlsx'):
                existing_df = pd.read_excel('data/produced_vehicles.xlsx')
            ...
            combined_df.to_excel('data/produced_vehicles.xlsx', index=False, engine='openpyxl')

        # 保存 departed_vehicles
        with FileLock("data/departed_vehicles.xlsx.lock"):
            departed_df = pd.DataFrame(departed_data, columns=['Time', 'Departed Vehicles'])
            if os.path.exists('data/departed_vehicles.xlsx'):
                existing_df = pd.read_excel('data/departed_vehicles.xlsx')
            ...
            combined_df.to_excel('data/departed_vehicles.xlsx', index=False, engine='openpyxl')
    ...
    def run_episode(self, max_steps=-1, exp=None):
        ...
        while ((max_steps > -1 and traci.simulation.getCurrentTime() < max_steps) or max_steps <= -1) and (
            ...
            #                     engine='openpyxl')
            with FileLock("RewardFunction/fit_reward.xlsx.lock"):
                fit_reward.to_excel('RewardFunction/fit_reward.xlsx', index=False, engine='openpyxl')
