...
class SUMOTrafficLights(Environment):
    ...
    def __init__(self, cfg_file, port=8813, use_gui=True):
    ...
    * the transitions between states are deterministic
    ...
    def run_episode(self, max_steps=-1, arq_tl='saida_tl.txt', exp=None):
        ...
        while ((max_steps > -1 and traci.simulation.getCurrentTime() < max_steps) or max_steps <= -1) and (
            ...
            # f = open("data.txt", "a")
            # msg = str(tick, " ")
            f.writelines(str(tick))
            # f.close()
            # for tlID in self.get_trafficlights_ID_list():
            for tlID in self.get_trafficlights_ID_list():
                if tlID == "2":# or tlID == "5" or tlID == "4" or tlID == "8":
                    # print(tlID)
                    f.writelines(" ")
                    f.writelines(str(self.calculate_stopped_queue_length(tlID)))
                    ...
                    if traci.trafficlight.getPhase(tlID) == 0 or traci.trafficlight.getPhase(tlID) == 3:
                        ...
                        if tlID == "2" and traci.trafficlight.getPhase(tlID) == 3:
                            currentQueue[int(tlID)] = 0
                        if tlID == "2" and traci.trafficlight.getPhase(tlID) == 0:
                            currentQueue[int(tlID)] = 100
                        if tlID == "5" and traci.trafficlight.getPhase(tlID) == 3:
                            currentQueue[int(tlID)] = 0
                        if tlID == "5" and traci.trafficlight.getPhase(tlID) == 0:
                            currentQueue[int(tlID)] = 0
                        if tlID == "4" and traci.trafficlight.getPhase(tlID) == 3:
                            currentQueue[int(tlID)] = 0
                        if tlID == "4" and traci.trafficlight.getPhase(tlID) == 0:
                            currentQueue[int(tlID)] = 0
                        if tlID == "8" and traci.trafficlight.getPhase(tlID) == 3:
                            currentQueue[int(tlID)] = 0
                        if tlID == "8" and traci.trafficlight.getPhase(tlID) == 0:
                            currentQueue[int(tlID)] = 0
    ...
    def __process_trafficlights_feedback(self, traffic_lights):
        ...
        for tlID in traffic_lights.keys():
            ...
            self._learners[str(tlID)].feedback_last(traffic_lights[tlID][0], traffic_lights[tlID][1],
                                                    traffic_lights[tlID][2], traffic_lights[tlID][3],
                                                    traffic_lights[tlID][4], traffic_lights[tlID][5])
