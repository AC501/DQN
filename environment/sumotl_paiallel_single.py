...
class SUMOTrafficLights(Environment):
    ...
    def __init__(self, cfg_file, port=8813, use_gui=False, batch_size=32):
        ...
        #     torch.load('D:\\tsc_ddqn_prb_1Con_new\\weights_0\\best_model_ddpm_GPT.pth'))
        ...
        self.rewardmodel = RewardPredictionModel(input_dim=30, output_dim=1)
    ...
    * the transitions between states are deterministic
    ...
    def save_vehicle_data(self, produced_data, departed_data):
        ...
        with FileLock("data_0/produced_vehicles.xlsx.lock"):
            produced_df = pd.DataFrame(produced_data, columns=['Time', 'Produced Vehicles'])
            if os.path.exists('data_0/produced_vehicles.xlsx'):
                existing_df = pd.read_excel('data_0/produced_vehicles.xlsx')
                combined_df = pd.concat([existing_df, produced_df], ignore_index=True)
                combined_df = combined_df.tail(25000000)  # 限制行数
            else:
                combined_df = produced_df
            combined_df.to_excel('data_0/produced_vehicles.xlsx', index=False, engine='openpyxl')

        # 保存 departed_vehicles
        with FileLock("data_0/departed_vehicles.xlsx.lock"):
            departed_df = pd.DataFrame(departed_data, columns=['Time', 'Departed Vehicles'])
            if os.path.exists('data_0/departed_vehicles.xlsx'):
                existing_df = pd.read_excel('data_0/departed_vehicles.xlsx')
                combined_df = pd.concat([existing_df, departed_df], ignore_index=True)
                combined_df = combined_df.tail(25000000)
            else:
                combined_df = departed_df
            combined_df.to_excel('data_0/departed_vehicles.xlsx', index=False, engine='openpyxl')
    ...
    def run_episode(self, max_steps=-1, exp=None):
        ...
        while ((max_steps > -1 and traci.simulation.getCurrentTime() < max_steps) or max_steps <= -1) and (
            ...
            for tlID in self.get_trafficlights_ID_list():
                ...
                if len(fit_rewardfunction_df) > max_rows:
                    ...
                    fit_rewardfunction_df = fit_rewardfunction_df[1:]
            ...
            #                     engine='openpyxl')
            with FileLock("RewardFunction/fit_reward_0.xlsx.lock"):
                fit_reward.to_excel('RewardFunction/fit_reward_0.xlsx', index=False, engine='openpyxl')

            # 读取 Excel 文件
            # fit_df = pd.read_excel('RewardFunction/fit_reward.xlsx', engine='openpyxl')
            with FileLock("RewardFunction/fit_reward_0.xlsx.lock"):
                fit_df = pd.read_excel('RewardFunction/fit_reward_0.xlsx', engine='openpyxl')
            ...
            state_columns = [f'state_{tlid}' for tlid in range(1)]
            ...
            state_data = result[['state_0']].values
            # 将每个状态的数据展开为一维数组
            ...
            # print("单条状态内容:", flattened_states[0])  # 是字符串
            # print("拆分后状态向量:", flattened_state[0])  # 是 list
            # print("最终flattened_state shape:", flattened_state.shape)
            ...
            for tlID in self.get_trafficlights_ID_list():
                if os.path.exists(file_path):
                    GenReward = pd.read_excel("data_0/generated_reward.xlsx")
                    # 从文件读取奖励
                    ...
                    if traci.trafficlight.getPhase(tlID) == 0:
                        r = np.random.uniform(-5, 5)  # 随机生成-5到5之间的浮点数
        ...
        # 保存为 Excel 文件
        df_traffic_light.to_excel("data_0/traffic_light_phases.xlsx", index=False)
