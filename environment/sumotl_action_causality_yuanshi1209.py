...
#from RewardFunction.FunctionFitting import RewardPredictionModel

'''

2024.12.9拟合reward的时候，是包括state和reward的
'''
...
class SUMOTrafficLights(Environment):
    ...
    def __init__(self, cfg_file, port=8813, use_gui=False, batch_size=32):
        ...
        # #加载回报函数拟合模型
        # self.rewardmodel = RewardPredictionModel(input_dim = 100,output_dim = 4)
        # self.rewardmodel.load_state_dict(torch.load('D:\\tsc_ddqn_prb_1Con_new\\RewardFunction\\reward_prediction_model.pth'))
    ...
    * the transitions between states are deterministic
    ...
    def run_episode(self, max_steps=-1, exp=None):
        ...
        while ((max_steps > -1 and traci.simulation.getCurrentTime() < max_steps) or max_steps <= -1) and (
            ...
            fit_reward = pd.DataFrame(fit_rewardfunction_df, columns=['tlid', 'fit_state_reward'])

            # 保存为Excel文件
            fit_reward.to_excel('D:/tsc_ddqn_prb_1Con_new/RewardFunction/fit_reward.xlsx', index=False,
                                engine='openpyxl')
            # 读取 Excel 文件
            fit_df = pd.read_excel('D:/tsc_ddqn_prb_1Con_new/RewardFunction/fit_reward.xlsx')

            # 预处理 state_reward 列，去除方括号并保留逗号
            fit_df['fit_state_reward'] = fit_df['fit_state_reward'].apply(lambda x: str(x).strip('[]'))
            # 创建一个空的 DataFrame 用于保存结果
            result = pd.DataFrame()
            ...
            for tlid in range(4):
                ...
                filtered_data['state'] = filtered_data['fit_state_reward'].apply(
                    lambda x: x.rsplit(',', 1)[0])  # 获取 state 部分
                filtered_data['reward'] = filtered_data['fit_state_reward'].apply(
            ...
            for tlID in self.get_trafficlights_ID_list():
                ...
                # state rward一起
                state_reward = new_state[int(tlID)]+[r]
                CPMData.append((tlID, state_reward ))


                # #state reward fenkai
                # state__ = new_state[int(tlID)]
                # reward__ = r
                # CPMData.append((tlID, state__,reward__ ))
                ...
                if traci.trafficlight.getPhase(tlID) == 0 or traci.trafficlight.getPhase(tlID) == 3:
                    ...
                    if self.__trafficlights[tlID]["greenTime"] > (minGreenTime - 1) and \
                        ...
                        # # #DDPM_GPT
                        # #
                        # new_states = torch.tensor([new_state[int(tlID)]], dtype=torch.float32).to(self.device) # 替换为你的新状态数据
                        # t = torch.tensor([self.__trafficlights[tlID]['step']], dtype=torch.int64).to(self.device) # 示例时间步
                        #
                        # # 正向扩散
                        # noisy_states = self.model(new_states, t)
                        #
                        # # 逆向扩散并预测奖励
                        # reward = self.model.reverse(noisy_states, t).item()
                        #print("预测的奖励:", reward)
        ...
        self.save_vehicle_data(produced_vehicles_data, departed_vehicles_data, slow_vehicles_data)
