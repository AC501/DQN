...
class SUMOTrafficLights(Environment):
    ...
    def __init__(self, cfg_file, port=8813, use_gui=False, batch_size=32):
        ...
        self.model.load_state_dict(torch.load('D:\\tsc_ddqn_prb_1Con_new\\weights\\best_model_ddpm_GPT_ctde1s.pth'))
    ...
    * the transitions between states are deterministic
    ...
    def run_episode(self, max_steps=-1,  exp=None):
        ...
        cted_state = [0] * len(self.get_trafficlights_ID_list())
        ...
        while ((max_steps > -1 and traci.simulation.getCurrentTime() < max_steps) or max_steps <= -1) and (
            ...
            traci.simulationStep()
            all_states = []  # 用于存储所有灯的状态
            for tlID in self.get_trafficlights_ID_list():
                # 进行仿真步骤
                current_phase = traci.trafficlight.getPhase(tlID)
                # ns, we = self.control_traffic(tlID)
                # queNS_cted, queEW_cted = self.calculate_stopped_queue_length(tlID)
                # cted_state[int(tlID)] = [current_phase,ns,we,queNS_cted,queEW_cted]
                #
                # all_states.append(cted_state[int(tlID)])  # 添加到列表中
                if current_phase == 0 or current_phase == 3:
                    ...
                    #每5s计算过车数
                    # idPhase, duration, total_NS, total_EW = self.get_traffic_state(tlID,self.__trafficlights[tlID]['step'])
                    ...
                    new_state[int(tlID)] = [idPhase, ns, we, queNS, queEW]
                    # all_states.append(new_state[int(tlID)])  # 添加到列表中
                ...
                # 将所有灯的状态添加到 ctde_data 列表中
            # ctde_data.append(all_states)  # 添加整体状态
        ...
        # # 文件路径
        # file_path = 'data/diffusion_data_1s.xlsx'
        # # 尝试追加数据到已有的 Excel 文件
        # try:
        ...
        # # 将四个灯的状态合并为一个嵌套列表，并存入单列
        # # 将 ctde_data 转换为 DataFrame，每行对应一个 all_states 列表
        # ctde_data1 = pd.DataFrame({'nex_state': ctde_data})
        # ctde_data1.to_excel('data/diffusion_data_ctde.xlsx', index=False)
