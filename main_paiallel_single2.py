...
from environment.sumotl_paiallel_single import SUMOTrafficLights
...
class CustomDDPMScheduler:
    ...
    def add_noise(self, original_samples, noise, timesteps):
        ...
        '''
        xₜ = √αₜ · x₀ + √(1 - αₜ) · ε
        xₜ: 第 t 步的带噪样本（noisy sample）
        x₀: 原始样本
        ε: 高斯噪声
        αₜ: 不同时间步的"保留比例"，越往后越小（噪声越多）
        '''
        # 确保所有输入都在正确的设备上
...
class DiffusionModel(nn.Module):
    def __init__(self, input_dim, condition_dim, hidden_dim=512, num_heads=8, num_lstm_layers=1,dropout = 0.2):
        ...
        self.lstm = nn.LSTM(
            ...
            hidden_size=self.hidden_dim,
            num_layers=num_lstm_layers,
            batch_first=True,   # 表示输入数据格式是 [batch, time, feature]
...
# 在添加噪声之前，先计算每个样本的权重
def calculate_weights(batch_c):
    # 将HC值归一化到0-1之间
    min_hc = torch.min(batch_c)
    max_hc = torch.max(batch_c)
    normalized_hc = (batch_c - min_hc) / (max_hc - min_hc + 1e-6)
    # 将HC值转换为权重，可以使用指数函数使差异更明显
    weights = torch.exp(normalized_hc)
    # 归一化权重，使其和为1
...
def train_diffusion_model(model, dataloader, optimizer, num_epochs, device):
    # 用于记录训练损失
    losses = []
    if os.path.exists("CPM_0/BestDDPModel.pth"):
        model.load_state_dict(torch.load("CPM_0/BestDDPModel.pth"))
    ...
    best_model_path = "CPM_0/BestDDPModel.pth"
    ...
    for epoch in range(num_epochs):
        ...
        for batch_x, batch_c in dataloader:
            ...
            # 计算样本权重
            sample_weights = calculate_weights(batch_c)  # batch_c是条件（HC值）
            ...
            # 计算带权重的均方误差损失
            weighted_errors = (pred_noise - noise) ** 2
            # print("batch_c.shape:", batch_c.shape)
            # print("pred_noise.shape:", pred_noise.shape)
            # print("noise.shape:", noise.shape)
            # print("weighted_errors.shape:", weighted_errors.shape)
            # print("sample_weights.shape:", sample_weights.shape)
            # weighted_loss = torch.mean(weighted_errors * sample_weights.view(-1, 1, 1))  # 调整权重的维度以匹配误差
            weighted_loss = torch.mean(weighted_errors * sample_weights)
            loss = weighted_loss
...
def compute_epistemic_uncertainty(model, condition, num_models=5, num_samples=10, device="cuda"):
    ...
    # 对每个特征维度计算模型之间预测均值的方差，再取所有特征维度上的平均
...
def generate_new_samples(model, condition, num_samples, device):
    ...
    model.eval()
    # 生成初始随机噪声
    # 创建 shape = [样本数, 时间步, 特征维度] 的随机噪声张量
    x = torch.randn(num_samples, 100).to(device)
    x = x.unsqueeze(-1)  # shape: [32, 5, 1]
    # 广播到与 x 相同的形状
    # x = x.expand(-1, -1, 4)  # shape: [32, 5, 4]
    x = x.expand(-1, -1, 1)  # shape: [32, 5, 4]

    # 将条件信息（如 y=3.5）复制为每个时间步共享的形式，形状为 [num_samples, 100, condition_dim]
    condition_expanded = condition.expand(num_samples, 100, -1).to(device)
    # 逐步去噪过程
    generated_samples = []
...
def run_episode_paiallel(i):
    port = find_free_port()
    env = SUMOTrafficLights('nets_0/fxy.sumocfg',
                            port,
                            False,
                            32)
    learners = {}
...
def main():
    ...
    epoches = 5
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    hc_means_file = 'data_0/hc_means.xlsx'
    plot_file = 'data_0/hc_means_plot.png'
    best_con_file = 'data_0/best_con.xlsx'
    # 确定断点位置
    start_epoch = 0
    ...
    for epoch in range(start_epoch,epoches):
        ...
        # # 动态确定n_episodes
        cpm_final_path = 'data_0/CPMData_final.xlsx'
        try:
        ...
        # with Pool(processes=num_workers) as pool:  # 根据CPU核心数调整
        #     pool.map(
        #         run_episode_paiallel,
        #         range(n_episodes)
        #         )
        ...
        #sys.exit()
        # 读取state和reward数据
        SRData = pd.read_excel('data_0/CPMData_final.xlsx')
        # 读取对应时间内区域产生的车辆数
        ProductData = pd.read_excel('data_0/produced_vehicles.xlsx')
        # 读取对应时间内区域离开的车辆数
        LeaveData = pd.read_excel('data_0/departed_vehicles.xlsx')
        ...
        ProductData["HC"] = (
                LeaveData["Departed Vehicles"] - ProductData["Produced Vehicles"] - M * is_congested
        )

        # 标记每一行属于哪个块
        n_blocks = len(ProductData) // block_size
        ProductData["block_id"] = [i for i in range(n_blocks) for _ in range(block_size)]

        # 标记是否保留每个块（均值 <= -2.3 的块保留）
        ProductData["keep"] = ProductData.groupby("block_id")["HC"].transform(lambda x: x.mean() <= 300)
        keep_mask = ProductData["keep"].values

        # 应用同样的 mask 到3个文件中
        SRData_del = SRData[keep_mask].reset_index(drop=True)
        ProductData_del = ProductData[keep_mask].drop(columns=["block_id", "keep", "HC"]).reset_index(drop=True)
        LeaveData_del = LeaveData[keep_mask].reset_index(drop=True)

        # 保存为新的文件
        SRData_del.to_excel("data_0/CPMData_final_del.xlsx", index=False)
        ProductData_del.to_excel("data_0/produced_vehicles_del.xlsx", index=False)
        LeaveData_del.to_excel("data_0/departed_vehicles_del.xlsx", index=False)

        print(f" 删除完成：共删除 {n_blocks - (len(SRData_del) // block_size)} 个块")

        # 重新读取经过筛选后的数据
        SRData = pd.read_excel('data_0/CPMData_final_del.xlsx')
        # 创建新的列来保存每列的最后一个元素
        #保存reward
        ...
        # 读取对应时间内区域产生的车辆数
        ProductData = pd.read_excel('data_0/produced_vehicles_del.xlsx')
        # 读取对应时间内区域离开的车辆数
        LeaveData = pd.read_excel('data_0/departed_vehicles_del.xlsx')
        ...
        # # 创建筛选条件：HC >= 0
        # mask = ProductData["HC"] >= 0
        #
        # # 应用筛选条件到 ProductData 和 SRData
        # ProductData_filtered = ProductData[mask]
        # SRData_filtered = SRData[mask].copy()  # 使用 copy() 避免 SettingWithCopyWarning
        #
        # # 确保 SRData_filtered 的 'y' 列与筛选后的 ProductData 一致
        ...
        # 分割数据为块并计算y的平均
        # reshape前：shape(？个样本，每个样本有？个特征)
        # reshape后：shape(？个块，每个块有？个样本，每个样本有？个特征)
        input_rewards = rewards.reshape((num_blocks, block_size, rewards.shape[1]))
        # reshape前：shape(？个样本，每个样本有？个特征)
        # reshape后：shape(？个块，每个块的平均y值为？)
        input_y = y.reshape((num_blocks, block_size, -1)).mean(axis=1)

        print("\n修改前:每个块的平均HC值:")
        for i in range(num_blocks):
            print(f"块 {i+1}: {input_y[i][0]:.3f}")
            
        # 使用最后一个块的平均值替换所有块的平均值
        last_block_mean = input_y[-1][0]
        print(f"\n平均值 {last_block_mean:.3f} 替换")
        input_y = np.full_like(input_y, last_block_mean)
        ...
        num_500s_blocks = total_samples // 500
        
        # 截断数据以匹配500s
        rewards = rewards[:(num_500s_blocks * 500), :]
        y = y[:(num_500s_blocks * 500), :]
        
        # 重塑数据为 (样本数, 500时间步, 特征数)
        input_rewards = rewards.reshape((num_500s_blocks, 500, rewards.shape[1]))
        
        # 使用最后一个块的平均值作为整个500s的平均值
        last_block_mean = input_y[-1][0]  # 获取最后一个100s块的平均值
        input_y = np.full((num_500s_blocks, 1), last_block_mean)  # 所有500s样本都使用这个值
        
        print(f"\n整合样本,平均值定义:{last_block_mean:.3f}")
        ...
        # 构建候选条件集（示例使用5个缩放因子）
        candidate_conditions = build_candidate_conditions(current_max_y)  # [5, 1]

        # 计算每个候选条件的UaE得分
        uaes = []
        for cond in candidate_conditions:
            # 扩展条件维度：[1,1] -> [1,5,1]（匹配模型输入）
            expanded_cond = cond.unsqueeze(0).expand(1, 100, -1).to(device)
            # 计算认知不确定性
            epistemic_uncert = compute_epistemic_uncertainty(model, expanded_cond)
            # 计算UaE得分
            uae_score = cond.item() - epistemic_uncert
            uaes.append(uae_score)

        # 选择得分最高的条件
        best_cond_idx = np.argmax(uaes)
        best_condition = candidate_conditions[best_cond_idx]

        # 使用最优条件生成样本
        expanded_best_cond = best_condition.unsqueeze(0).expand(num_samples, 100, -1).to(device)  # [num_samples,5,1]
        generated_samples = generate_new_samples(model, expanded_best_cond, num_samples, device)
        ...
        # 将生成的数据转换为整数
        generated_samples_cpu = np.round(generated_samples_cpu).astype(int)  # 四舍五入并转换为整数
        # Reshape the array to (100, 4)
        # reshaped_data = generated_samples_cpu.reshape(-1, 4)
        reshaped_data = generated_samples_cpu.reshape(-1, 1)
        ...
        # Save to Excel
        file_path = 'data_0/generated_reward.xlsx'
