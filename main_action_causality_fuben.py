...
from RewardFunction.FunctionFitting_fuben2 import rewardfitmain
...
# 读取对应时间内区域产生的车辆数
ProductData = pd.read_excel(r'D:\tsc_ddqn_prb_xiugaimainkuangjia\data\produced_vehicles.xlsx')
...
def main():
    ...
    for epoch in range(epoches):
        ...
        generated_samples_tensor = torch.tensor(generated_samples, dtype=torch.int32).to(device)
        print('generated_samples_tensor',generated_samples_tensor.shape)
        print('states_rewards_tensor',states_rewards_tensor.shape)
        new_states_rewards_tensor = torch.cat((states_rewards_tensor, generated_samples_tensor), dim=0)
        # 扩展 new_condition 的维度，使它在维度1上的大小与 y_tensor 一致
        new_condition = new_condition.repeat(1, 5)  # 将维度1从 1 扩展为 5
        print('new_condition',new_condition.shape)
        print('y_tensor',y_tensor.shape)

        new_y_tensor = torch.cat((y_tensor, new_condition), dim=0)  # 这里的 generated_y_tensor 是生成的目标条件

        # 使用新的数据集进行训练
        new_dataset = TrafficDataset(new_states_rewards_tensor, new_y_tensor)
        new_data_loader = DataLoader(new_dataset, batch_size=32, shuffle=True)

        # 继续训练扩散模型
        train_diffusion_model(model, new_data_loader, optimizer, num_epochs, device)
